...
//		String accept = "Server accepted!";
//		sendSocketMessage(accept);
//		BaseHelper.printLine("Server send to " +  socket.getLocalPort() + " " + accept);
		while(true) {			
...
//			String reply = message + "OK";
//			sendSocketMessage(reply);
		}
...
//			JSONPObject json = BaseUtils.obj.readValue(mess.getValue().getBytes(), JSONPObject.class);
...
//		if(BaseConstant.SOCKET_EVENT_CREATE_ROOM.equals(message)) {
//			
//		} else
//		if(message.isEmpty()) {
//			return;
//		}
//		else {
//			MessageSocket ms = BaseUtils.obj.readValue(message.getBytes(), MessageSocket.class);
//			BaseHelper.printLine("action: " + ms.getAction() + " value: " + ms.getValue());
